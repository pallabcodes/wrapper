...
export const validateInventory = (inventory: Inventory): DomainResult<Inventory> => {
  return pipe(
    ...
    E.chain((validInventory: any) => {
      // Cast to proper Inventory type with required fields
      const typedInventory: Inventory = {
        stockQuantity: validInventory.stockQuantity,
        reservedQuantity: validInventory.reservedQuantity ?? 0,
        reorderLevel: validInventory.reorderLevel ?? 10,
        maxStockLevel: validInventory.maxStockLevel,
        trackInventory: validInventory.trackInventory ?? true,
        allowBackorder: validInventory.allowBackorder ?? false,
        lastStockUpdate: validInventory.lastStockUpdate
      }
      
      return pipe(
        validateBusinessRule(
          'reserved-not-exceed-stock',
          typedInventory.reservedQuantity <= typedInventory.stockQuantity,
          'Reserved quantity cannot exceed stock quantity',
          { inventory: typedInventory }
        ),
        E.map(() => typedInventory)
      )
    })
  )
}
