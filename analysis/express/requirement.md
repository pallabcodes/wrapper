...
We are a high-growth engineer-first culture with cracked devs that works uprising sillicon valley produc-based companies and as well top tier product based companies like Atlassian, Stripe, Google, Amazon, Shopfiy and Sillicon Valley Product based companies etc.

# Requirements: (class could be used or skipped but no OOP patterns only functional design patterns)

- The folder structure + architechture we have recevied from client (QUITE READY) but off course we have told througly review and give our honest feedback for the scale we will working on

- We have received one of the projects where we have been told "EXPRESS.JS"

- (VERY IMPRORANT): This sandbox/folder structure + architechture should be able to handle 100 as well 1M+ concurrent users and MAU could be 1000 or 10M+, DAU could be 1000 and as well 100K-1M

- We must ensure a proper hybrid or modern monolithic architechture where any of its module could be instantly extracted for serverless and microservices architechture
...
# Objective: Enterprise Laravel E-commerce Architecture
...
- Can write C++ add on if needed to use here and integrate within this project
...
WE CAN PLACES ALL OF THESE PROPERLY WITHIN #Express directory

# Objectives: We have given two PHASES here

- Extract everything that could be extracted from the source-code of Express i.e. at #Express-source i.e. 

- Either universally repurposable eleshwere or I can use native implmenation or built my c++ add on that will work with Node.js therefore Express and customize it to fit my project's usecase e.g. I want to do somthing with autoloader or I might take a native feture tweak or create wrapper or modify as needed for better or simpler etc.
...
GOAL: is to use `Express` than even the creator himslef didn't quite expect or explore so for that I need to know nitty-gritty of it
