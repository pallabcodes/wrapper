...
import { UserCreatedEvent } from '../../../../../events/auth/user-created.event';
import { UserEmailChangedEvent } from '../../../../../events/auth/user-email-changed.event';
...
export class UserAggregate extends AggregateRoot {
  ...
  static reconstituteFromEvents(events: any[]): UserAggregate {
    if (events.length === 0) {
    ...
    const user = new UserAggregate(
      ...
      Email.create(firstEvent.email),
      firstEvent.name,
      firstEvent.passwordHash,
      firstEvent.role as UserRole
    );
