...
# StreamVerse User Service - Docker Manager Script
# Handles all Docker operations for the user service
...
# Configuration
SERVICE_NAME="streamverse-user-service"
...
wait_for_service() {
    ...
    log_error "User service failed to start within expected time"
    return 1
}
...
# Kafka/Redis Configuration (for future use)
...
start_service() {
    log_header "STARTING USER SERVICE"
    ...
    log_info "Building and starting user service..."
    npm run build
    npm run start:dev &
    SERVICE_PID=$!
    ...
    if wait_for_service; then
        log_success "User service started successfully (PID: $SERVICE_PID)"
...
stop_service() {
    log_header "STOPPING USER SERVICE"

    if [ -f ".service.pid" ]; then
        SERVICE_PID=$(cat .service.pid)
        if kill -0 $SERVICE_PID 2>/dev/null; then
            log_info "Stopping user service (PID: $SERVICE_PID)..."
            kill $SERVICE_PID 2>/dev/null || true
            sleep 2
            if kill -0 $SERVICE_PID 2>/dev/null; then
            ...
            log_success "User service stopped"
        else
...
test_service() {
    ...
    echo ""
    echo -e "${CYAN}Testing User Registration:${NC}"
    REGISTER_RESPONSE=$(curl -s -X POST http://localhost:$SERVICE_PORT/users/register \
...
# Show usage
show_usage() {
    log_header "STREAMVERSE USER SERVICE - DOCKER MANAGER"
    echo "Usage: $0 <command>"
    echo ""
    echo "Commands:"
    echo "  setup        - Full setup: install deps, start DB, start service"
    echo "  start        - Start all services (DB + user service)"
    ...
    echo "  svc-start    - Start user service only"
    echo "  svc-stop     - Stop user service only"
    echo ""
    echo "Maintenance:"
