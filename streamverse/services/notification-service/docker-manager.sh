...
# StreamVerse Notification Service - Docker Manager Script
# Handles all Docker operations for the notification service
...
# Configuration
SERVICE_NAME="streamverse-notification-service"
...
wait_for_service() {
    log_info "Waiting for notification service to be ready..."
    local max_attempts=30
    local attempt=1

    while [ $attempt -le $max_attempts ]; do
        if curl -s http://localhost:$SERVICE_PORT/health >/dev/null 2>&1; then
            log_success "Notification service is ready!"
            return 0
        fi
    ...
    log_error "Notification service failed to start within expected time"
    return 1
}
...
# Redis Configuration (for rate limiting - if needed)
...
start_service() {
    log_header "STARTING NOTIFICATION SERVICE"
    ...
    log_info "Building and starting notification service..."
    npm run build
    npm run start:dev &
    SERVICE_PID=$!
    ...
    if wait_for_service; then
        log_success "Notification service started successfully (PID: $SERVICE_PID)"
        ...
        echo -e "  ${CYAN}Notification API:${NC} http://localhost:$SERVICE_PORT/notifications"
        ...
        log_error "Failed to start notification service"
...
stop_service() {
    log_header "STOPPING NOTIFICATION SERVICE"

    if [ -f ".service.pid" ]; then
        SERVICE_PID=$(cat .service.pid)
        if kill -0 $SERVICE_PID 2>/dev/null; then
            log_info "Stopping notification service (PID: $SERVICE_PID)..."
            kill $SERVICE_PID 2>/dev/null || true
            sleep 2
            if kill -0 $SERVICE_PID 2>/dev/null; then
            ...
            log_success "Notification service stopped"
        else
...
test_service() {
    ...
    echo ""
    echo -e "${CYAN}Testing Notification Creation:${NC}"
    NOTIFICATION_RESPONSE=$(curl -s -X POST http://localhost:$SERVICE_PORT/notifications \
    ...
    if echo "$NOTIFICATION_RESPONSE" | jq -e '.notificationId' >/dev/null 2>&1; then
        echo -e "  ${GREEN}✅ Notification creation working${NC}"
        NOTIFICATION_ID=$(echo "$NOTIFICATION_RESPONSE" | jq -r '.notificationId')
        echo -e "  ${CYAN}Created notification:${NC} $NOTIFICATION_ID"
    else
    ...
    echo ""
    echo -e "${CYAN}Testing Template Notification:${NC}"
    TEMPLATE_RESPONSE=$(curl -s -X POST http://localhost:$SERVICE_PORT/notifications/welcome \
        -H "Content-Type: application/json" \
        -d '{
          "userId": "test-user-123",
    ...
    if echo "$TEMPLATE_RESPONSE" | jq -e '.notificationId' >/dev/null 2>&1; then
        echo -e "  ${GREEN}✅ Template notification working${NC}"
    else
        echo -e "  ${YELLOW}⚠️ Template notification failed (providers may not be configured)${NC}"
...
# Show usage
show_usage() {
    log_header "STREAMVERSE NOTIFICATION SERVICE - DOCKER MANAGER"
    echo "Usage: $0 <command>"
    echo ""
    echo "Commands:"
    echo "  setup        - Full setup: install deps, start DB, start service"
    echo "  start        - Start all services (DB + notification service)"
    ...
    echo "  svc-start    - Start notification service only"
    echo "  svc-stop     - Stop notification service only"
    echo ""
    echo "Maintenance:"
